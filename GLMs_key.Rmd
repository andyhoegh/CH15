...
*Furthermore, the Poisson distribution also has a single parameter that controls the mean __and__ variance of the distribution, so the standard deviation implied by the model would be $\sqrt(\mu_i)$*
...
\begin{align}
  y_i & \sim Negative Binomial(\mu_i, \phi)\\
  \log(\mu_i) & = X_i \beta
\end{align}
